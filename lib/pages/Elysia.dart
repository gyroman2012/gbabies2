...
import 'package:firephotogallery/galleries/ElysiaGallery.dart';
import 'package:flutter/material.dart';

class ElysiaUploadScreen extends StatefulWidget {
  @override
  _ElysiaUploadScreenState createState() => _ElysiaUploadScreenState();
}

class _ElysiaUploadScreenState extends State<ElysiaUploadScreen> {
  ...
  uploadFileToStorage(File file) {
    UploadTask task = _firebaseStorage
        .ref()
        .child("elysia/${DateTime.now().toString()}")
        .putFile(file);
    return task;
  }

  writeImageUrlToFireStore(imageUrl) {
    _firebaseFirestore.collection("elysia").add({"url": imageUrl}).whenComplete(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          IconButton(
              ...
              onPressed: () {
                Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => ElysiaGalleryScreen()));
              })
        ],
      ),
      ...
      body: uploadedTasks.length == 0
          ? Center(child: Text("Please tap on photo button to upload images"))
          : ListView.separated(
              itemBuilder: (context, index) {
                return StreamBuilder<TaskSnapshot>(
                  builder: (context, snapShot) {
                    return snapShot.hasError
                        ? Text("There was an error in uploading file")
                        : snapShot.hasData
                            ? ListTile(
                                title: Text(
